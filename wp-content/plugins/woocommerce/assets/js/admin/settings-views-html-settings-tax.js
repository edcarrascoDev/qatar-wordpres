    ...
    var rowTemplate = wp.template('wc-tax-table-row'),
        ...
        getFilteredRates: function() {
          ...
          if (search.length) {
            rates = _.filter(rates, function(rate) {
              var search_text = _.toArray(rate)
                .join(' ')
                .toLowerCase();
      ...
      WCTaxTableViewConstructor = Backbone.View.extend({
        ...
        onAddNewRow: function(event) {
          ...
          if ($current.length) {
            ...
            reordered_rates = _.map(rates_to_reorder, function(rate) {
              rate.tax_rate_order++;
              changes[rate.tax_rate_id] = _.extend(changes[rate.tax_rate_id] || {}, {
        ...
        onExport: function(event) {
          var csv_data =
            'data:application/csv;charset=utf-8,' + data.strings.csv_data_cols.join(',') + '\n';
