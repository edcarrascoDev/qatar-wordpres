...
jQuery(function($) {
  ...
  var wc_meta_boxes_product_variations_actions = {
    ...
    init: function() {
      $('#variable_product_options')
        ...
        .on('click', 'button.add_price_for_variations', this.open_modal_to_set_variations_price)
      ...
      $.post(woocommerce_admin_meta_boxes.ajax_url, data, function(response) {
        if (response.error) {
          ...
          this_page_url = this_page_url.replace(
            'post-new.php?',
            'post.php?post=' + woocommerce_admin_meta_boxes.post_id + '&action=edit&',
          );
    ...
    reload: function() {
      wc_meta_boxes_product_variations_ajax
        .load_variations(1)
        .then(wc_meta_boxes_product_variations_ajax.show_hide_variation_empty_state);
    ...
    variations_loaded: function(event, needsUpdate) {
      ...
      if (!needsUpdate) {
        ...
        $('.woocommerce_variations .variation-needs-update', wrapper).removeClass(
          'variation-needs-update',
        );

        // Disable cancel and save buttons
        $('button.cancel-variation-changes, button.save-variation-changes', wrapper).attr(
          'disabled',
          'disabled',
        );
      }
    ...
    variation_row_indexes: function() {
      var wrapper = $('#variable_product_options').find('.woocommerce_variations'),
        ...
        offset = parseInt(
          (current_page - 1) * woocommerce_admin_meta_boxes_variations.variations_per_page,
          10,
        );
      ...
      $('.woocommerce_variations .woocommerce_variation').each(function(index, el) {
        $('.variation_menu_order', el)
          .val(
            parseInt($(el).index('.woocommerce_variations .woocommerce_variation'), 10) +
              1 +
              offset,
          )
          .trigger('change');
  ...
  var wc_meta_boxes_product_variations_media = {
    ...
    add_image: function(event) {
      ...
      if ($button.is('.remove')) {
        $('.upload_image_id', wc_meta_boxes_product_variations_media.setting_variation_image)
          .val('')
          .trigger('change');
  ...
  var wc_meta_boxes_product_variations_ajax = {
    ...
    initial_load: function() {
      if (
        0 ===
        $('#variable_product_options').find('.woocommerce_variations .woocommerce_variation').length
      ) {
      ...
      if (0 < need_update.length) {
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            need_update.removeClass('variation-needs-update');
            $('button.cancel-variation-changes, button.save-variation-changes').attr(
              'disabled',
              'disabled',
            );
    ...
    link_all_variations: function() {
      ...
      if (window.confirm(woocommerce_admin_meta_boxes_variations.i18n_link_all_variations)) {
        ...
        $.post(woocommerce_admin_meta_boxes_variations.ajax_url, data, function(response) {
          ...
          window.wp.data.dispatch('core/notices').createSuccessNotice(message, { icon: '🎉' });
      ...
      switch (do_variation_action) {
        ...
        case 'variable_sale_price_decrease':
          ...
          if (value != null) {
            if (value.indexOf('%') >= 0) {
              data.value =
                accounting.unformat(value.replace(/\%/, ''), woocommerce_admin.mon_decimal_point) +
                '%';
            } else {
