    ...
    var $table = $('.wc-shipping-zone-methods'),
      ...
      ShippingMethodView = Backbone.View.extend({
        ...
        initialize: function() {
          ...
          $(document.body).on(
            'change',
            '.wc-shipping-zone-method-selector select',
            this.onChangeShippingMethodSelector,
          );
          $(document.body).on(
            'click',
            ...
            $.each(methods, function(id, rowData) {
              ...
              if (!rowData.has_settings) {
                $tr
                  ...
                  .replaceWith(
                    '<span>' + $tr.find('.wc-shipping-zone-method-title > a').text() + '</span>',
                  );
        ...
        onToggleEnabled: function(event) {
          ...
          changes.methods = changes.methods || { methods: {} };
          changes.methods[instance_id] = _.extend(changes.methods[instance_id] || {}, {
            enabled: enabled,
          });
          model.set('methods', methods);
          model.logChanges(changes);
        ...
        updateModelOnSort: function(event) {
          ...
          _.each(methods, function(method) {
            ...
            var new_position = parseInt(
              $table.find('tr[data-id="' + method.instance_id + '"]').index() + 1,
              10,
            );
        ...
        moveHTMLHelpTips: function(html) {
          ...
          labels.each(i => {
            ...
            if (id === 'woocommerce_free_shipping_ignore_discounts') {
              ...
              if (
                fieldset.length &&
                fieldset.find('.wc-shipping-zone-method-fields-help-text').length === 0
              ) {
                fieldset.append(
        ...
        onChangeShippingMethodSelector: function() {
          ...
          $(this)
            .parent()
            .find('.wc-shipping-zone-method-description')
            .remove();
          $(this).after(
            '<div class="wc-shipping-zone-method-description">' + description + '</div>',
          );
        },
