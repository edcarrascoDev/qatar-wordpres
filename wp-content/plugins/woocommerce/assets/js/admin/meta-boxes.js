jQuery(function($) {
   ...
   */
  jQuery.is_attribute_or_variation_empty = function(attributes_and_variations_data) {
    var has_empty_fields = false;
    attributes_and_variations_data.each(function() {
      var $this = $(this);
      // Check if the field is checkbox or a search field.
      if ($this.hasClass('checkbox') || $this.filter('[class*=search__field]').length) {
        return;
      }

      var is_empty = $this.is('select') ? $this.find(':selected').length === 0 : !$this.val();
      if (is_empty) {
  ...
  jQuery.maybe_disable_save_button = function() {
    ...
    if (jQuery.is_attribute_or_variation_empty(attributes_and_variations_data)) {
