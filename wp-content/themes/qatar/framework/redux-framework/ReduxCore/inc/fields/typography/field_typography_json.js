...
(function($) {
    ...
    redux.field_objects.typography.init = function(selector, skipCheck) {
        if (!selector) {
            selector = $(document)
                ...
                .find('.redux-container-typography:visible');
        }
        ...
        $(selector).each(function() {
            ...
            el.each(function() {
                ...
                $(this)
                    .find('.redux-typography-container')
                    .each(function() {
                ...
                $(this)
                    ...
                    .on('select2-opening', function(e) {
                        // Get field ID
                        var thisID = $(this)
                            .parents('.redux-container-typography:first')
                            .attr('data-id');
                        ...
                        if (
                            usingGoogleFonts == 1 ||
                            (usingGoogleFonts === true && redux.googlefonts !== undefined)
                        ) {
                        ...
                        // get placeholder
                        var selFamily = $('#' + thisID + ' #' + thisID + '-family').attr(
                            'placeholder',
                        );
                        if (!selFamily) {
                            selFamily = null;
                    ...
                    .on('select2-clearing', function(val, choice) {
                        var thisID = $(this)
                            .parents('.redux-container-typography:first')
                            .attr('data-id');

                        $('#' + thisID + ' #' + thisID + '-family').attr('data-value', '');
                        $('#' + thisID + ' #' + thisID + '-family').attr(
                            'placeholder',
                            'Font Family',
                        );
    ...
    redux.field_objects.typography.select = function(selector, skipCheck) {
        ...
        if ($(selector).hasClass('redux-typography-family')) {
            ...
            if (google === true) {
                ...
                $.each(details.variants, function(index, variant) {
                    if (
                        variant.id === style ||
                        redux.field_objects.typography.size(details.variants) === 1
                    ) {
                ...
                $.each(details.subsets, function(index, subset) {
                    if (
                        subset.id === script ||
                        redux.field_objects.typography.size(details.subsets) === 1
                    ) {
                        selected = ' selected="selected"';
                        script = subset.id;
                        $('#' + mainID + ' input.typography-subsets').val(script);
                ...
                $('#' + mainID + ' .redux-typography-subsets')
                    .parent()
                    .fadeIn('fast');
        ...
        if (
            $('#' + mainID + " select.redux-typography-style option[value='" + style + "']")
                .length === 0
        ) {
            style = '';
        ...
        if (
            $('#' + mainID + " select.redux-typography-subsets option[value='" + script + "']")
                .length === 0
        ) {
            script = '';
        ...
        if (
            family !== null &&
            family !== 'inherit' &&
            $('#' + mainID).hasClass('typography-initialized')
        ) {
        ...
        if ($('#' + mainID).hasClass('typography-initialized')) {
            ...
            if (color) {
                ...
                $('#' + mainID + ' .typography-preview').css(
                    'background-color',
                    redux.field_objects.typography.contrastColour(color),
                );
            }
