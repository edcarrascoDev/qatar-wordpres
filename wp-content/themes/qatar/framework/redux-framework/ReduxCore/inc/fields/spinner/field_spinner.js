    ...
    redux.field_objects.spinner.init = function(selector) {
        if (!selector) {
            selector = $(document)
                ...
                .find('.redux-container-spinner:visible');
        }
        ...
        $(selector).each(function() {
            ...
            el.find('.redux_spinner').each(function() {
                //slider init
                var spinner = $(this)
                    .find('.spinner-input')
                    .data();
            ...
            el.find('.spinner-input').keyup(function() {
                $(this).addClass('spinnerInputChange');
            });

            el.find('.spinner-input').focus(function() {
                redux.field_objects.spinner.clean($(this).val(), $(this));
            });

            el.find('.spinner-input').typeWatch({
                callback: function(value) {
                    redux.field_objects.spinner.clean(value, $(this));
                },
