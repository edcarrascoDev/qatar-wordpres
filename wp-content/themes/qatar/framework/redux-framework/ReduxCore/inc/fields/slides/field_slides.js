    ...
    redux.field_objects.slides.init = function(selector) {
        if (!selector) {
            selector = $(document)
                ...
                .find('.redux-container-slides:visible');
        }
        ...
        $(selector).each(function() {
            ...
            el.find('.redux-slides-remove').live('click', function() {
                redux_change($(this));
                ...
                var slideCount = $(this)
                    .parents('.redux-container-slides:first')
                    .find('.redux-slides-accordion-group').length;

                if (slideCount > 1) {
                    $(this)
                        .parents('.redux-slides-accordion-group:first')
                        .slideUp('medium', function() {
                            $(this).remove();
                        });
                } else {
                    var content_new_title = $(this)
                        .parent('.redux-slides-accordion')
                        .data('new-content-title');
                    ...
                    $(this)
                        .parents('.redux-container-slides:first')
                        .find('.redux-slides-accordion-group:last')
                        .find('.redux-slides-header')
                        .text(content_new_title);
                }
            });

            //el.find( '.redux-slides-add' ).click(
            el.find('.redux-slides-add')
                .off('click')
                .click(function() {
                    var newSlide = $(this)
                        .prev()
                    ...
                    $(newSlide)
                        ...
                        .text('')
                        .append(
                            '<span class="redux-slides-header">' +
                                content_new_title +
                                '</span><span class="ui-accordion-header-icon ui-icon ui-icon-plus"></span>',
                        );
                    $(this)
                        .prev()
                        .append(newSlide);
