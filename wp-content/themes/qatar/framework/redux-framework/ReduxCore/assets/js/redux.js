...
(function($) {
    ...
    $.redux.ajax_save = function(button) {
        ...
        if (redux.fields.hasOwnProperty('editor')) {
            $.each(redux.fields.editor, function($key, $index) {
        ...
        jQuery.ajax({
            ...
            success: function(response) {
                if (response.action && response.action == 'reload') {
                    ...
                    var $save_notice = $(document.getElementById('redux_notification_bar')).find(
                        '.saved_notice',
                    );
    ...
    $.redux.initEvents = function() {
        $('.redux-presets-bar').on('click', function() {
        ...
        $(
            ...
            if (
                ($('#toplevel_page_' + redux.args.slug).hasClass('wp-menu-open') ||
        ...
        $('.expand_options').click(function(e) {
            ...
            if ($(container).hasClass('fully-expanded')) {
                ...
                $('.redux-container:first')
                    .find('#' + tab + '_section_group')
                    .fadeIn(200, function() {
    ...
    $.redux.checkRequired = function() {
        ...
        $('.redux-container td > fieldset:empty,td > div:empty')
            .parent()
            .parent()
            .hide();
    };
    ...
    $.redux.tabCheck = function() {
        $('.redux-group-tab-link-a').click(function() {
            ...
            var oldid = el
                .find('.redux-group-tab-link-li.active:first .redux-group-tab-link-a')
                .data('rel');
            ...
            if (
                el.find('#' + relid + '_section_group_li').parents('.redux-group-tab-link-li')
                    .length
            ) {
                var parentID = el
            ...
            el.find('#toplevel_page_' + redux.args.slug + ' .wp-submenu a.current').removeClass(
                'current',
            );
            el.find('#toplevel_page_' + redux.args.slug + ' .wp-submenu li.current').removeClass(
                'current',
            );

            el.find('#toplevel_page_' + redux.args.slug + ' .wp-submenu a').each(function() {
                var url = $(this)
                    .attr('href')
                    .split('&tab=');
            ...
            if (
                el.find('#' + oldid + '_section_group_li').find('#' + oldid + '_section_group_li')
                    .length
            ) {
                ...
                if (el.find('#' + oldid + '_section_group_li').find('ul.subsection').length) {
                    ...
                    el.find('#' + oldid + '_section_group_li').parents('ul.subsection').length
                ) {
            ...
            $('#toplevel_page_' + redux.args.slug)
                .find('.current')
                .removeClass('current');
        });
        ...
        if (
            $.cookie('redux_current_tab') === null ||
            typeof $.cookie('redux_current_tab') === 'undefined' ||
            sTab.length === 0
        ) {
            $('.redux-container')
    ...
    $.redux.initFields = function() {
        $('.redux-group-tab:visible')
            ...
            .each(function() {
                ...
                //console.log(type);
                if (
                    typeof redux.field_objects != 'undefined' &&
    ...
    $.redux.notices = function() {
        if (redux.errors && redux.errors.errors) {
            $.each(redux.errors.errors, function(sectionID, sectionArray) {
                $.each(sectionArray.errors, function(key, value) {
                    $('#' + redux.args.opt_name + '-' + value.id).addClass('redux-field-error');
                    if (
                        $('#' + redux.args.opt_name + '-' + value.id)
                            .parent()
                            .find('.redux-th-error').length === 0
                    ) {
                        $('#' + redux.args.opt_name + '-' + value.id).append(
                            '<div class="redux-th-error">' + value.msg + '</div>',
                        );
                    } else {
                        $('#' + redux.args.opt_name + '-' + value.id)
                            .parent()
                ...
                if (totalErrors > 0) {
                    ...
                    container.find('.redux-group-tab').each(function() {
                        ...
                        if (total > 0) {
                            ...
                            container
                                .find('.redux-group-tab-link-a[data-key="' + sectionID + '"]')
        ...
        if (redux.warnings && redux.warnings.warnings) {
            $.each(redux.warnings.warnings, function(sectionID, sectionArray) {
                $.each(sectionArray.warnings, function(key, value) {
                    $('#' + redux.args.opt_name + '-' + value.id).addClass('redux-field-warning');
                    if (
                        $('#' + redux.args.opt_name + '-' + value.id)
                            .parent()
                            .find('.redux-th-warning').length === 0
                    ) {
                        $('#' + redux.args.opt_name + '-' + value.id).append(
                            '<div class="redux-th-warning">' + value.msg + '</div>',
                        );
                    } else {
                        $('#' + redux.args.opt_name + '-' + value.id)
                            .parent()
        ...
        $.each(redux.folds, function(i, v) {
            ...
            if (v == 'hide') {
                ...
                if (fieldset.hasClass('redux-container-raw')) {
                    var rawTable = fieldset
                        .parents()
                        .find('table#' + redux.args.opt_name + '-' + i);
    ...
    $.redux.get_container_value = function(id) {
        ...
        if (
            value !== null &&
            typeof value === 'object' &&
            value.hasOwnProperty(redux.args.opt_name)
        ) {
        ...
        $.each(redux.required[id], function(child, dependents) {
            ...
            if (show === true) {
                ...
                if (childFieldset.hasClass('redux-container-raw')) {
                    var rawTable = childFieldset
                        .parents()
                        .find('table#' + redux.args.opt_name + '-' + child);
    ...
    $.redux.required_recursive_hide = function(id) {
        ...
        toFade.fadeOut(50, function() {
            ...
            if ($('#' + redux.args.opt_name + '-' + id).hasClass('redux-container-raw')) {
                var rawTable = $('#' + redux.args.opt_name + '-' + id)
                    .parents()
                    .find('table#' + redux.args.opt_name + '-' + id);
    ...
    $.redux.check_parents_dependencies = function(id) {
        ...
        if (redux.required_child.hasOwnProperty(id)) {
            $.each(redux.required_child[id], function(i, parentData) {
                if (
                    $('#' + redux.args.opt_name + '-' + parentData.parent)
                        .parents('tr:first')
                        .hasClass('hide')
                ) {
                    show = false;
...
function redux_change(variable) {
    ...
    var parentID = jQuery(variable)
        .closest('.redux-group-tab')
        .attr('id');
    ...
    if (
        ...
        if (errorCount <= 0) {
            ...
            if (errorsLeft <= 0) {
                th.find('.redux-menu-error:first')
                    .fadeOut()
                    .remove();
            } else {
    ...
    if (
        ...
        if (warningCount <= 0) {
            //console.log('HERE');
            jQuery('#' + parentID + '_li .redux-menu-warning')
                .fadeOut('fast')
                .remove();
            ...
            if (warningsLeft <= 0) {
                th.find('.redux-menu-warning:first')
                    .fadeOut()
                    .remove();
            } else {
