...
<div class="cart__totals <?php echo ( WC()->customer->has_calculated_shipping() ) ? 'calculated_shipping' : ''; ?>">
	...
	<div class="shop-table shop-table--responsive">
		...
		<div  class="shop-table__tr cart-subtotal">
			...
			<div class="shop-table__td" data-title="<?php esc_attr_e( 'Subtotal', 'woocommerce' ); ?>"><?php wc_cart_totals_subtotal_html(); ?></div>
		</div >
		...
		<?php foreach ( WC()->cart->get_coupons() as $code => $coupon ) : ?>
			<div  class="shop-table__tr cart-discount coupon-<?php echo esc_attr( sanitize_title( $code ) ); ?>">
				...
				<div class="shop-table__td" data-title="<?php echo esc_attr( wc_cart_totals_coupon_label( $coupon, false ) ); ?>"><?php wc_cart_totals_coupon_html( $coupon ); ?></div>
			</div >
		<?php endforeach; ?>
		...
		<?php elseif ( WC()->cart->needs_shipping() && 'yes' === get_option( 'woocommerce_enable_shipping_calc' ) ) : ?>
			...
			<div  class="shop-table__tr shipping">
				<div class="shop-table__th"> <?php esc_html_e( 'Shipping', 'woocommerce' ); ?></div>
				<div class="shop-table__td" data-title="<?php esc_attr_e( 'Shipping', 'woocommerce' ); ?>"><?php woocommerce_shipping_calculator(); ?></div>
			</div >
		...
		<?php foreach ( WC()->cart->get_fees() as $fee ) : ?>
			<div  class="shop-table__tr fee">
				...
				<div class="shop-table__td" data-title="<?php echo esc_attr( $fee->name ); ?>"><?php wc_cart_totals_fee_html( $fee ); ?></div>
			</div >
		<?php endforeach; ?>
		...
		if ( wc_tax_enabled() && ! WC()->cart->display_prices_including_tax() ) {
			...
			if ( 'itemized' === get_option( 'woocommerce_tax_total_display' ) ) {
				...
				<div  class="shop-table__tr tax-total">
					<div class="shop-table__th"> <?php echo esc_html( WC()->countries->tax_or_vat() ) . $estimated_text; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
					<div class="shop-table__td" data-title="<?php echo esc_attr( WC()->countries->tax_or_vat() ); ?>"><?php wc_cart_totals_taxes_total_html(); ?></div>
				</div >
				<?php
			}
		}
		...
		<div  class="shop-table__tr order-total">
			...
			<div class="shop-table__td" data-title="<?php esc_attr_e( 'Total', 'woocommerce' ); ?>"><?php wc_cart_totals_order_total_html(); ?></div>
		</div >
	...
	<div class="wc-proceed-to-checkout  mt-25 ">
